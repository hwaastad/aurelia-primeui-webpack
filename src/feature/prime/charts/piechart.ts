...
export class PieChartComponent {
  ...
  @bindable scaleBeginAtZero: boolean = false;
  ...
  @bindable percentageInnerCutout: number = 0;
  ...
  constructor(private element: Element,private bindingEngine: BindingEngine) {
    this.value=[];
  /*  obsLoc.getArrayObserver(this.value)
    .subscribe(splices => console.log(splices));*/
    this.subscription = this.bindingEngine.collectionObserver(this.value).subscribe(splices => console.log(splices));
