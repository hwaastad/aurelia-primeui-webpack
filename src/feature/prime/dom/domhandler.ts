export class DomHandler {
  ...
  public addAttributes(element: any, attrs) {
    let keys = Object.keys(attrs);
    keys.forEach(k => {
      if (!element.getAttribute(k)) {
        //this.addedAttributes[k] = attrs[k];
        element.setAttribute(k, attrs[k]);
      } else if (element.getAttribute(k) !== attrs[k]) {
        element.setAttribute(k, attrs[k]);
      }
    });
  }

  public removeAttributes(element: any, attrs) {
